...
In fact, many of your Aunts Sue have many of these. You put the wrapping from the gift into the MFCSAM. It beeps inquisitively at you a few times and then prints out 
...
def parse_input(filename):
    ...
    for line in open(filename).read().strip().split("\n"):
        ...
        _, number = sue.strip().split(" ")
        sues[number.strip()] = []
        for part in data.strip().split(","):
            key, value = part.split(":")
            sues[number.strip()].append({key.strip(): int(value.strip())})
    return sues
...
class d16:
    ...
    def p1(self):
        ...
        must_find = []
        for line in part_1_list.strip().split("\n"):
            key, value = line.strip().split(":")
            must_find.append({key.strip(): int(value.strip())})


        sues = parse_input('input')
        for sue_number in sues:
            characteristics = sues[sue_number]
            good = True
            for dictionary in characteristics:
                if dictionary not in must_find:
                    good = False

            if good:
                print("\tAnswer to part 1:", sue_number)
                break
    def p2(self):
        """
--- Part Two ---
...
In particular, the cats and trees readings 
indicates that there are greater than that many 
...
"""
        ...
        for line in part_1_list.strip().split("\n"):
            key, value = line.strip().split(":")
            must_find.append({key.strip(): int(value.strip())})
            must_find_dict[key.strip()] = int(value.strip())
        ...
        for sue_number in sues:
            ...
            good = True
            for dictionary in characteristics:
                dictionary_key = tuple(dictionary.keys())[0] 
                if dictionary_key in ['cats', 'trees']:
                    if dictionary[dictionary_key] <= must_find_dict[dictionary_key]:
                        good = False

                elif dictionary_key in ['pomeranians', 'goldfish']:
                    if dictionary[dictionary_key] >= must_find_dict[dictionary_key]:
                        good = False

                elif dictionary not in must_find:
                    good = False

            if good:
                print("\tAnswer to part 2:", sue_number)
                break
        pass
